    ...
    def preprocessing(self):
        ...
        while updated:
            ...
            for (r, c) in dummy:
                ...
                if r > 0:
                    if (r-1, c) in fixed_pieces:
                        pass

                    else:
                        if (r-1, c) not in possible_actions:
                            possible_pieces = check_neighbour(r, c, r-1, c, 'upper', self.board, [])

                        else:
                            possible_pieces = check_neighbour(r, c, r-1, c, 'upper', self.board, possible_actions[(r - 1, c)])
                ...
                if r < rows - 1:
                    if (r+1, c) in fixed_pieces:
                        ...
                        if (r+1, c) not in possible_actions:
                            possible_pieces = check_neighbour(r, c, r+1, c, 'lower', self.board, [])

                        else:
                            possible_pieces = check_neighbour(r, c, r+1, c, 'lower', self.board, possible_actions[(r + 1, c)])
                ...
                if c > 0:
                    if (r, c-1) in fixed_pieces:
                        pass

                    else:
                        if (r, c - 1) not in possible_actions:
                            possible_pieces = check_neighbour(r, c, r, c - 1,'left', self.board, [])

                        else:
                            possible_pieces = check_neighbour(r, c, r, c - 1, 'left', self.board, possible_actions[(r, c - 1)])
                ...
                if c < cols - 1:
                    if (r, c+1) in fixed_pieces:
                        ...
                        if (r, c + 1) not in possible_actions:
                            possible_pieces = check_neighbour(r, c, r, c + 1,'right', self.board, [])

                        else:
                            possible_pieces = check_neighbour(r, c, r, c + 1, 'right', self.board, possible_actions[(r, c + 1)])
...
class PipeMania(Problem):
    ...
    def h(self, node: Node) -> int:
        ...
        for row in range(rows):
            for col in range(cols):
                counter = 0
                upper, lower = node.state.board.adjacent_vertical_values(
                    row, col)
                left, right = node.state.board.adjacent_horizontal_values(
                    row, col)
